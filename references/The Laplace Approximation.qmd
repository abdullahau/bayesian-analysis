...
`.laplace_sample()` method returns also unormalized target log density (`lp__`) and unnormalized density of the Laplace approximation to the posterior (`lp_q__`) for the draws from the normal approximation. These can be used to compute importance ratios `np.exp(lp__-lp_q__)` which can be used to do importance resampling. We can do the importance resampling using the posterior package.
