...
m4_5_model = utils.StanQuap('stan_models/m4_5', m4_5, data, algorithm = 'Newton')
```
...
m4_6_model = utils.StanQuap('stan_models/m4_6', m4_6, data, algorithm = 'Newton')
```
...
def stan_data_dict(degree):
    ...
    X = formula.get_model_matrix(d).to_numpy()
    weight_seq = pd.DataFrame({'weight_s': np.linspace(-2.2, 2, 30)})
    X_tilde = formula.get_model_matrix(weight_seq).to_numpy()
    return {'N': len(d),
            'K': 4,
            'height': d['height'].tolist(),
            'X': X.tolist(),
...
# m4_6a_model = utils.StanQuap('stan_models/m4_6a', m4_6a, data = stan_data_dict(3), algorithm = 'Newton')

test_stan = utils.Stan('stan_models/m4_6a', m4_6a)
test_bs = utils.BridgeStan('stan_models/m4_6a', stan_data_dict(3))
test_bs.param_names()
test_opt = test_stan.optimize(data = stan_data_dict(3), algorithm = 'Newton', jacobian=False)
# test_opt.stan_variables()

test_bs.param_num()
test_bs.param_unc_names()
test_opt._metadata.stan_vars
```
