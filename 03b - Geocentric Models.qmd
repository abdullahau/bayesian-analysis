...
mu = m4_3_model.link(lm_func=lm, data=d2['weight'].to_numpy())
print(mu)
```
...
weight_s_seq = np.linspace(-2.2, 2, 30)
mu = m4_5_model.link(lm_func=lm, data=weight_s_seq)
...
def plot_post_sim():
    ...
    plt.fill_between(weight_s_seq, mu_plo, mu_phi, alpha=0.2)
    plt.fill_between(weight_s_seq, sim_plo, sim_phi, alpha=0.2)
...
weight_s_seq = np.linspace(-2.2, 2, 30)
mu = m4_6_model.link(lm_func=lm, data=weight_s_seq)
mu_df = pd.DataFrame(
    {str(round(weight, 2)): mu[:, i] for i, weight in enumerate(weight_s_seq)}
    )
mu_summary = utils.precis(mu_df, index_name='Weight_s')
sim_height = m4_6_model.sim(data=data)['y_tilde']
sim_height_df = pd.DataFrame(
    {str(round(weight, 2)): sim_height[:, i] for i, weight in enumerate(weight_s_seq)}
    )
sim_summary = utils.precis(sim_height_df, index_name='Weight_s')

def plot_post_sim():
    plt.plot(d.weight_s, d.height, 'o', fillstyle='none', label='Observed Data', color='k')
    plt.plot(weight_s_seq, mu_summary['Mean'], 'k', label='Mean')
    plt.fill_between(weight_s_seq, mu_summary['5.5%'], mu_summary['94.5%'], alpha=0.2)
    plt.fill_between(weight_s_seq, sim_summary['5.5%'], sim_summary['94.5%'], alpha=0.2)
    plt.xlabel('Weight (Standardized)')
    plt.ylabel('Height (cm)')    
...
def plot_poly():
    ...
    for i, ax in enumerate(axes):
        ...
        def lm(post, X):
            return (X @ post['b'].T).T
        mu = model.link(lm_func=lm, data=X, n=1000)
        mu_df = pd.DataFrame(
            {str(round(weight, 2)): mu[:, i] for i, weight in enumerate(weight_seq)}
            )
        mu_summary = utils.precis(mu_df, index_name='Weight_s')
        sim = model.sim(data=stan_data_dict(i+1))['y_tilde']
        sim_height_df = pd.DataFrame(
            {str(round(weight, 2)): sim[:, i] for i, weight in enumerate(weight_seq)}
            )
        sim_summary = utils.precis(sim_height_df, index_name='Weight_s')

        ax.plot(d.weight_s, d.height, 'o', fillstyle='none', label='Observed Data', color='k', alpha=0.3)
        ax.plot(weight_s_seq, mu_summary['Mean'], 'k', label='Mean')
        ax.fill_between(weight_s_seq, mu_summary['5.5%'], mu_summary['94.5%'], alpha=0.2)
        ax.fill_between(weight_s_seq, sim_summary['5.5%'], sim_summary['94.5%'], alpha=0.2)
        ax.set(xlabel='Weight (Standardized)', ylabel='Height (cm)', xlim=(-2.2,2), ylim=(40, 185))
        ax.set_title(title[i])

utils.inline_plot(plot_poly)
```
...
def bspline_regression_plots(data, num_knots, degree):
    ...
    mu_df = pd.DataFrame(
        {str(round(weight, 2)): post['mu'][:, i] for i, weight in enumerate(data.year)}
        )
    mu_summary = utils.precis(mu_df, index_name='Weight_s')    
    axs[2].plot(data.year, data.doy, 'o', color='pink', alpha=0.6, zorder=2)
    axs[2].fill_between(data.year, mu_summary['5.5%'], mu_summary['94.5%'], alpha=0.6, zorder=3)
    axs[2].set(xlabel='Year', ylabel='Day in Year')
    
    fig.tight_layout()

utils.inline_plot(bspline_regression_plots, data=d2, num_knots=5, degree=1)
```
...
utils.inline_plot(plot_weighted_basis)
```
...
```{python}
mu_df = pd.DataFrame(
    {str(round(weight, 2)): post['mu'][:, i] for i, weight in enumerate(d2.year)}
    )
mu_summary = utils.precis(mu_df, index_name='Weight_s')  
def plot_post():
    plt.figure(figsize=(8,3))
    plt.plot(d2.year, d2.doy, 'o', color='pink', alpha=0.6)
    plt.fill_between(d2.year, mu_summary['5.5%'], mu_summary['94.5%'], alpha=0.6)
    plt.xlabel('Year')
    plt.ylabel('Basis Value')
...
```{python}
utils.inline_plot(bspline_regression_plots, data=d2, num_knots=15, degree=3)
```
