...
To compute the quadratic approximation for the globe tossing data, we’ll use a mixture of tools from Stan:

1.  Stan's Optimization Method: MAP estimation using `model.optimize()` and inverting Hessian matrix obtained from the `fit.hessian_matrix()`.

2.  Laplace Approximation

Let us compute the quadratic approximation to the globe tossing data using both the methods:
...
globe_qa_model = utils.StanModel('stan_models/globe_qa', globe_qa)

toss_data = {'W': 6, 'L': 3}

# Optimize to get MAP estimate
globe_fit_opt = globe_qa_model.optimize(data=toss_data)

globe_fit_p = globe_fit_opt.stan_variables(inc_iterations=True)
print(globe_fit_p)
...
globe_qa_model = utils.StanModel('stan_models/globe_qa2', globe_qa2)

toss_data = {'W': 6, 'L': 3}

# Optimize to get MAP estimate
globe_fit_opt = globe_qa_model.optimize(data=toss_data, jacobian=False)

laplace = globe_qa_model.laplace_sample(data=toss_data, mode=globe_fit_opt, draws=1000, jacobian=False)

laplace.stan_variables()['p'].std()

globe_fit_opt.optimized_params_pd
...
sample_mcmc_globe = globe_qa_model.sample(data=toss_data, show_progress=False)
sample_mcmc_globe.summary()
```
