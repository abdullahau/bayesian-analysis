...
To compute the quadratic approximation for the globe tossing data, we’ll use Stan's `optimize()` and `laplace_sample()` algorithms.
...
# Optimize to get Maximum Likelihood Estimate (MLE)
...
lap_bin = globe_qa_model.laplace_sample(data=toss_data, jacobian=False, draws=4000)
...
def fit_pdf(p, model):
    lp = model.log_prob(params={'p': p}, data=toss_data, jacobian=False)['lp__'][0]
    return np.exp(lp)

p = expit(np.linspace(0,1,100))
np.vectorize(fit_pdf)(p, globe_qa_model).std()

globe_qa_model.log_prob(params={'p': 0.67}, data=toss_data)
```
