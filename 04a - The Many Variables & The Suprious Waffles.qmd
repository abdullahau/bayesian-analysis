...
def plot_residual():
    ...
    mu_M = m5_4_model.laplace_sample(draws=1000).stan_variable('mu')
    mu_A = m5_4a_model.laplace_sample(draws=1000).stan_variable('mu')
    
    mu_M_mean = np.mean(mu_M, axis=0)
    mu_M_resid = d.M - mu_M_mean
    
    mu_A_mean = np.mean(mu_A, axis=0)
    mu_A_resid = d.A - mu_A_mean
    
    _, axes = plt.subplots(1, 2, figsize=(9, 4))
    
    axes[0].plot(d.A, d.M, 'o', fillstyle='none')
    axes[0].plot(d.A, mu_M_mean, 'k')
    segments = [[(d.A[i], d.M[i]), (d.A[i], mu_M_mean[i])] for i in range(len(d.M))]
    axes[0].add_collection(collections.LineCollection(segments))
    axes[0].set(xlabel='Age at Marriage (std)', ylabel='Marriage Rate (std)')
    
    axes[1].plot(d.M, d.A, 'o', fillstyle='none')
    axes[1].plot(d.M, mu_A_mean, 'k')
    segments = [[(d.M[i], d.A[i]), (d.M[i], mu_A_mean[i])] for i in range(len(d.A))]
    axes[1].add_collection(collections.LineCollection(segments))
    axes[1].set(xlabel='Marriage Rate (std)', ylabel='Age at Marriage (std)')
