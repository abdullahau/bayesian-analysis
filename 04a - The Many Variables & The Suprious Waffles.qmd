...
format:
    html:
        ...
        -   text: Quarto Markdown
            icon: file-code
            href: https://github.com/abdullahau/bayesian-analysis/blob/main/04a%20-%20The%20Many%20Variables%20%26%20The%20Suprious%20Waffles.qmd
        html-math-method:
            method: mathjax
...
m5_1_model = utils.StanQuap('stan_models/m5_1', m5_1, data=data, 
                            generated_var=['y_tilde', 'a_sim', 'bA_sim', 'mu_sim', 'mu_tilde', 'mu'])
```
...
```{python}
def plot_priors():
    mu_sim = m5_1_model.laplace_sample(draws=100).stan_variable('mu_sim')
    plt.plot(A_seq, mu_sim.T, 'k', alpha=0.2)
    plt.xlim(-2,2)
...
def plot_post():
    def lm(post, predictor):
        return post['a'].reshape(-1,1) + post['bA'].reshape(-1,1) * predictor
    mu = m5_1_model.link(lm, A_seq, n=1000)
    mu_mean, mu_plo, mu_phi = utils.precis(mu)
    plt.plot(d.A, d.D, 'o', fillstyle='none', color='k', label='Observed Data')
    plt.plot(A_seq, mu_mean, 'k', label='Mean')
    plt.fill_between(A_seq, mu_plo, mu_phi, alpha=0.2, label='Uncertainty in Mean')
    plt.xlabel('Median Age Marriage (std)')
    plt.ylabel('Divorce Rate (std)')
    plt.legend()
...
If we inspect the `precis` output, we’ll see that posterior for $β_A$ is reliably negative, as seen in the plot above.

```{python}
m5_1_model.precis()
```

We can fit a similar regression for the divorce rate and marriage rate relationship:
...
m5_2_model = utils.StanQuap('stan_models/m5_2', m5_2, data=data, 
                            generated_var=['y_tilde', 'a_sim', 'bM_sim', 'mu_sim', 'mu_tilde', 'mu'])
...
def plot_post():
    def lm(post, predictor):
        return post['a'].reshape(-1,1) + post['bM'].reshape(-1,1) * predictor
    mu = m5_2_model.link(lm, M_seq, n=1000)
    mu_mean, mu_plo, mu_phi = utils.precis(mu)
    plt.plot(d.M, d.D, 'o', fillstyle='none', color='k', label='Observed Data')
    plt.plot(M_seq, mu_mean, 'k', label='Mean')
    plt.fill_between(M_seq, mu_plo, mu_phi, alpha=0.2, label='Uncertainty in Mean')
...
m5_dmat_model = utils.Stan('stan_models/m5_dmat', m5_dmat)
```
...
```{python}
def dmat(dataframe, regressors):
    if isinstance(regressors, list):
        regressors = " + ".join(regressors)
    formula = Formula(f"1 + {regressors}")
    return formula.get_model_matrix(dataframe).to_numpy()
```
...
m5_3_model = utils.StanQuap('stan_models/m5_3', m5_3, data=data, 
                            generated_var=['y_rep', 'mu'])

m5_3_model.precis().round(2)
```
...
def plot_forest():
    ...
    plt.hlines(y_range[3],
               xmin=m5_3_model.precis()['5.5%']['bA'], 
               xmax=m5_3_model.precis()['94.5%']['bA'], linewidth=2)
    plt.hlines(y_range[2],
               xmin=m5_1_model.precis()['5.5%']['bA'], 
               xmax=m5_1_model.precis()['94.5%']['bA'], linewidth=2)
    plt.hlines(y_range[1],
               xmin=m5_3_model.precis()['5.5%']['bM'], 
               xmax=m5_3_model.precis()['94.5%']['bM'], linewidth=2)
    plt.hlines(y_range[0],
               xmin=m5_2_model.precis()['5.5%']['bM'], 
               xmax=m5_2_model.precis()['94.5%']['bM'], linewidth=2)
    mean_range = np.array([m5_3_model.precis()['Mean']['bA'],
                           m5_1_model.precis()['Mean']['bA'],
                           m5_3_model.precis()['Mean']['bM'],
                           m5_2_model.precis()['Mean']['bM']])
    plt.plot(mean_range, y_range[::-1], 'o', fillstyle='none')
    plt.axvline(0, linestyle='--', linewidth=0.3)
    plt.yticks(y_range, labels=['bM - m5_2', 'bM - m5_3', 'bA - m5_1', 'bA - m5_3'])
    plt.xlabel('Estimate')
...
m5_4_model = utils.StanQuap('stan_models/m5_4', m5_4, data=data, 
                            generated_var=['y_rep', 'mu'])

m5_4a_model = utils.StanQuap('stan_models/m5_4a', m5_4a, data=data, 
                            generated_var=['y_rep', 'mu'])
```
...
def plot_residual():
    # Compute residuals
    mu_M_mean = m5_4_model.laplace_sample(draws=1000).stan_variable('mu').mean(axis=0)
    mu_A_mean = m5_4a_model.laplace_sample(draws=1000).stan_variable('mu').mean(axis=0)
    mu_M_resid, mu_A_resid = (d.M - mu_M_mean).to_numpy(), (d.A - mu_A_mean).to_numpy()
        
    # Generate mean divorce rate predictions
    def get_mu_D(resid):
        resid_dmat = np.ones((len(resid), 2))
        resid_dmat[:, 1] = resid
        data = {'N': len(d), 'K': 2, 'D': d.D.tolist(), 'X': resid_dmat.tolist()}
        return m5_dmat_model.laplace_sample(data=data, draws=1000).stan_variable('mu')
    
    mu_DM, mu_DA = get_mu_D(mu_M_resid), get_mu_D(mu_A_resid)
    
    fig, axes = plt.subplots(2, 2, figsize=(9, 9))
    
    def plot_residuals(ax, x, y, pred_mean, xlabel, ylabel):
        ax.plot(x, y, 'o', fillstyle='none')
        ax.plot(x, pred_mean, 'k')
        segments = [[(x[i], y[i]), (x[i], pred_mean[i])] for i in range(len(y))]
        ax.add_collection(collections.LineCollection(segments))
        ax.set(xlabel=xlabel, ylabel=ylabel)
    
    plot_residuals(axes[0, 0], d.A, d.M, mu_M_mean, 'Age at Marriage (std)', 'Marriage Rate (std)')
    plot_residuals(axes[0, 1], d.M, d.A, mu_A_mean, 'Marriage Rate (std)', 'Age at Marriage (std)')
    
    def plot_predictions(ax, resid, mu, xlabel, ylabel):
        mu_mean, mu_plo, mu_phi = utils.precis(mu)
        ax.plot(resid, d.D, 'o', fillstyle='none')
        sorted_index = np.argsort(resid)
        ax.plot(resid[sorted_index], mu_mean[sorted_index], 'k')
        ax.fill_between(resid[sorted_index], mu_plo[sorted_index], mu_phi[sorted_index], color='k', alpha=0.2)
        ax.axvline(0, linestyle='--', linewidth=0.2)
        ax.set(xlabel=xlabel, ylabel=ylabel)
    
    plot_predictions(axes[1, 0], mu_M_resid, mu_DM, 'Marriage Rate Residuals', 'Divorce Rate (std)')
    plot_predictions(axes[1, 1], mu_A_resid, mu_DA, 'Age at Marriage Residuals', 'Divorce Rate (std)')
...
```{python}
# call laplace_sample to directly obtain mu (linear model data)
# Not specifying new data so it uses original data
mu = m5_3_model.laplace_sample(draws=1000).stan_variable('mu') 

# summarize samples across cases
mu_mean, mu_plo, mu_phi = utils.precis(mu)

# simulate observations. Again no new data, so uses original data
D_sim = m5_3_model.sim(n=1000)['y_rep']
D_mean, D_plo, D_phi = utils.precis(D_sim)
```
...
def plot_post():
    ...
    ax.errorbar(d.D, 
                mu_mean, 
                yerr=(mu_mean - mu_plo, mu_phi - mu_mean), 
                fmt='none')
...
def plot_residual_state():
    ...
    ax.errorbar(d2.D_residual, d2.Loc, 
                 xerr=np.abs([np.abs(D_mean[idx] - mu_plo[idx]), np.abs(D_mean[idx] - mu_phi[idx])]),
                 fmt='C0o')
    
    ax.errorbar(d2.D_residual, d2.Loc, 
                 xerr=np.abs([np.abs(D_mean[idx] - D_plo[idx]), np.abs(D_mean[idx] - D_phi[idx])]),
                 fmt='C0o', alpha=0.2)
...
x_real = np.random.normal(size=100)                              # x_real as Gaussian with mean 0 and stddev 1
x_spur = np.random.normal(loc=x_real, size=100)                  # x_spur as Gaussian with mean=x_real
y = np.random.normal(loc=x_real, size=100)                       # y as Gaussian with mean=x_real
df = pd.DataFrame({'y': y, 'x_real': x_real, 'x_spur': x_spur})  # bind all together in data frame
```
...
generated quantities {
    ...
    // Simulating D for directly controlled M
    for (i in 1:N_tilde) {
...
m5_3_A_model = utils.StanQuap('stan_models/m5_3_A', m5_3_A, data=data, algorithm='LBFGS',
...
```{python}
# simulate M_tilde and then D_tilde, using A_seq
s = m5_3_A_model.sim(n=1000, select=['M_tilde', 'D_tilde'])
```
...
def plot_counterfactual():
    D_mean, D_plo, D_phi = utils.precis(s['D_tilde'])
    M_mean, M_plo, M_phi = utils.precis(s['M_tilde'])

    _, ax = plt.subplots(1, 2, figsize=(9, 4))
    
    ax[0].plot(seq, D_mean)
    ax[0].fill_between(seq, D_plo, D_phi, alpha=0.2)
    ax[0].set(xlabel='Manipulated A', 
              ylabel='Counterfactual D', 
              title='Total Counterfactual Effect of A on D')

    ax[1].plot(seq, M_mean)
    ax[1].fill_between(seq, M_plo, M_phi, alpha=0.2)
    ax[1].set(xlabel='Manipulated A', 
              ylabel='Counterfactual M', 
              title='Counterfactual Effect A -> M')
...
```{python}
# A_seq standardized to mean 26.1 and std dev 1.24
data = {'N': len(d),
        'A': d.A.tolist(),
        'M': d.M.tolist(),
        'D': d.D.tolist(),
        'N_tilde': 2,
        'A_seq': ((np.array([20,30])-26.1) / 1.24).tolist(),
        'M_seq': [0,0]}

s2 = m5_3_A_model.sim(data=data, n=1000, select=['M_tilde', 'D_tilde'])
(s2['D_tilde'][:, 1] - s2['D_tilde'][:, 0]).mean()
```
...
data = {'N': len(d),
        ...
        'D': d.D.tolist(),
        'N_tilde': len(seq),
        'A_seq': np.zeros(30).tolist(),
        'M_seq': seq.tolist()}

s = m5_3_A_model.sim(data=data, n=1000, select=['D_tilde_M'])

def plot_counterfactual():
    D_mean, D_plo, D_phi = utils.precis(s['D_tilde_M'])
    plt.plot(seq, D_mean)
    plt.fill_between(seq, D_plo, D_phi, alpha=0.2)
    plt.ylim(-2.1, 2)
    plt.xlabel('Manipulated M'), plt.ylabel('Counterfactual D')
...
```{python}
post = m5_3_A_model.extract_samples(n=1000)

M_sim = stats.norm.rvs(loc=post['aM'] + post['bAM'] * A_seq.reshape(-1,1), scale=post['sigma_M'])
M_sim.T
```
...
```{python}
D_sim = stats.norm.rvs(loc=post['a'] + post['bA'] * A_seq.reshape(-1,1) + post['bM'] * M_sim, scale=post['sigma_M'])
D_sim.T
```
...
```{python}
def plot_counterfactual():
    D_mean, D_phi, D_plo = utils.precis(D_sim.T)
    plt.plot(A_seq, D_mean)
    plt.fill_between(A_seq, D_plo, D_phi, alpha=0.2)
    plt.xlabel('Manipulated A'), plt.ylabel('Counterfactual D')
